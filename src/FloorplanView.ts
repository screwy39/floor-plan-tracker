import { AppState, ReferencePoint, WorldPoint } from './types.js';
...
export class FloorplanView {
   ...
   * Draw path polyline
   */
  private drawPath(path: WorldPoint[], calibration: AppState['calibration']): void {
    if (!calibration || path.length < 2) return;
   ...
   */
  private drawPose(pose: { x: number; y: number; theta: number }, calibration: AppState['calibration']): void {
    ...
    this.ctx.lineTo(
      arrowX - arrowheadLength * Math.cos(pose.theta - arrowheadAngle),
      arrowY - arrowheadLength * Math.sin(pose.theta - arrowheadAngle)
    );
    this.ctx.moveTo(arrowX, arrowY);
    this.ctx.lineTo(
      arrowX - arrowheadLength * Math.cos(pose.theta + arrowheadAngle),
   ...
   */
  private drawReferencePoint(ref: ReferencePoint, calibration: AppState['calibration']): void {
    if (!calibration) return;
   ...
   */
  screenToImage(screenX: number, screenY: number, transform: AppState['canvasTransform']): { u: number; v: number } {
