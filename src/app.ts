...
import { ImagePoint, ReferencePoint, CalibrationData } from './types.js';
...
export class FloorPlanTrackerApp {
  ...
  // Calibration state
  private calibrationMode: 'none' | 'two-point' | 'start-point' | 'reference-point' = 'none';
  private twoPointCalibration: { point1: ImagePoint | null; point2: ImagePoint | null } = {
    point1: null,
    point2: null,
  };
  ...
  private touchStartPoints: Array<{ x: number; y: number }> = [];
  ...
  private setupUI(): void {
    ...
    const fileInput = document.getElementById('floorplan-input') as HTMLInputElement;
    ...
    // Settings
    const correctionModeSelect = document.getElementById('correction-mode') as HTMLSelectElement;
    correctionModeSelect?.addEventListener('change', (e) => {
      const mode = (e.target as HTMLSelectElement).value as "gps-wifi" | "wifi-only" | "manual-only";
      this.stateStore.setCorrectionMode(mode);
    });

    const showRefsToggle = document.getElementById('show-refs') as HTMLInputElement;
  ...
  private async handleFloorPlanLoad(e: Event): Promise<void> {
  ...
  private setFloorPlanImage(img: HTMLImageElement): void {
  ...
  private async loadPdfAsImage(file: File): Promise<HTMLImageElement | null> {
  ...
  private fitImageToCanvas(img: HTMLImageElement): void {
  ...
  private startTwoPointCalibration(): void {
  ...
  private startStartPointCalibration(): void {
    this.calibrationMode = 'start-point';
    this.showMessage('Tap your current location on the floor plan');
  }

  private async startReferencePointCalibration(): Promise<void> {
  ...
  private handleCalibrationTap(x: number, y: number): void {
    const state = this.stateStore.getState();
  ...
  private promptTwoPointDistance(): void {
    ...
    const state = this.stateStore.getState();
    if (!this.twoPointCalibration.point1 || !this.twoPointCalibration.point2) return;

    // Use first point as origin for now
    const calibration = Calibration.computeFromTwoPoints(
      this.twoPointCalibration.point1,
      this.twoPointCalibration.point2,
      distance,
      this.twoPointCalibration.point1
    );
  ...
  private setStartPoint(imagePoint: ImagePoint): void {
    const state = this.stateStore.getState();
  ...
  private async addReferencePoint(imagePoint: ImagePoint): Promise<void> {
    const state = this.stateStore.getState();
    if (!state.calibration) {
  ...
  private async startCorrectionLoop(): Promise<void> {
    const loop = async () => {
      const state = this.stateStore.getState();
      ...
      if (state.calibration && state.correctionMode !== 'manual-only') {
        ...
        if (externalEstimate) {
          const correction = SensorsFusion.computeCorrection(
            { x: drPose.x, y: drPose.y },
            externalEstimate,
  ...
  private updateReferenceList(): void {
    ...
    state.referencePoints.forEach(ref => {
      ...
      const sources = [];
      if (ref.geoSource) sources.push(ref.geoSource.toUpperCase());
      if (ref.wifiFingerprintId) sources.push('Wi-Fi');
      ...
      const snapBtn = item.querySelector('.ref-snap-btn') as HTMLButtonElement;
